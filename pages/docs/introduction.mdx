...
**Payment Aggrigator** a first-class variant API library for [TailwindCSS](https://tailwindcss.com/).

## Key features

#### Zero Runtime

**Payment Aggrigator** is a zero runtime library, it doesn't add any extra code to your bundle, it's just a set of functions that will generate the classes you need.

#### Variants

Since [Stitches](https://stitches.dev) introduced variants, we became big fans of them, they are a great way to create a consistent design system, so we created **Payment Aggrigator** to bring them to TailwindCSS.
...
**Payment Aggrigator** allows you to apply variants to different screen sizes.
...
**Payment Aggrigator** provides a `class` / `className` prop for overriding classes on any component.
...
**Payment Aggrigator** allows you to compose components using the `extend` parameter. It automatically merges the `classes`, `slots`, `variants`, `defaultVariants` and `compoundVariants` of the extended component.
...
**Payment Aggrigator** is built with developer experience in mind, it provides a great autocomplete experience thanks to the fully-typed API, so when using TypeScript, `slots`, `values`, and `breakpoints` will be auto-completed for you.


#### Automatic conflict resolution

**Payment Aggrigator** implements [**tailwind-merge**](https://github.com/dcastil/tailwind-merge) under the hood, so it will efficiently merge your classes, so you don't have to worry about TailwindCSS class conflicts.
...
**Payment Aggrigator** is heavily inspired by [Stitches](https://stitches.dev) and [CVA](https://github.com/joe-bell/cva).
