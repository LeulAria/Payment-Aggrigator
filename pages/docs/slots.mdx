...
<Callout type="info">
  <div className="text-sm">
    **Payment Aggrigator** will automatically select the correct variant for each
    slot.
  </div>
</Callout>
...
```js copy
import { tv, cxBase } from "tailwind-variants";
...
const pagination = tv({
  slots: {
    base: "flex flex-wrap relative gap-1 max-w-fit",
    item: cxBase(baseItem, 'data-[active="true"]:bg-blue-500 data-[active="true"]:text-white'),
    prev: baseItem,
    next: baseItem,
  },
  variants: {
...
**Payment Aggrigator** also allows you to apply responsive variants to your slots. See more about responsive variants [here](/docs/responsive-variants).
